...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        ...
        def read_input(paths_):
            ...
            raw_input = tf.image.resize_images(raw_input, [256, 256], method=tf.image.ResizeMethod.AREA)
...
def generator(generator_inputs):
    ...
    # 13: [batch, 256, 256, 64] => [batch, 256, 256, 1] (no skip layer)
    conv8 = gen_conv(deconv2, 1, 3, 1, "conv8", function="tanh")
    layers.append(conv8)
...
def create_model(inputs, targets):
    def discriminator(inputs, targets):
        ...
        #noise
        noise = tf.random_normal(shape=tf.shape(targets), mean=0.0, stddev=noise_dev, dtype=tf.float32)
        ...
        #layer 1 => [128 * 126 * 32]
        g_1 = discrim_conv(con_inputs, 32, 2, "g_1")
        layers.append(g_1)
        #layer 2 => [64 * 64 * 64]
        g_2 = discrim_conv(g_1, 64, 2, "g_2")
        layers.append(g_2)
        #layer 3 => [32 * 32 * 128]
        g_3 = discrim_conv(g_2, 128, 2, "g_3")
        layers.append(g_3)
        #layer 4 => [16 * 16 * 256]
        g_4 = discrim_conv(g_3, 512, 2, "g_4")
        layers.append(g_4)
        #layer 5 => [8 * 8 * 512]
        g_5 = discrim_conv(g_4, 512, 2, "g_5")
        layers.append(g_5)
        #layer 6 => [4, 4, 512]
        g_6 = discrim_conv(g_5, 512, 2, "g_6", function="sigmoid")
        layers.append(g_6)
        #layer 7 => [2, 2, 512]
#        g_7 = discrim_conv(g_6, 512, 2, "g_7")
#        layers.append(g_7)
        #layer 8 => [1, 1, 512]
#        g_8 = discrim_conv(g_7, 512, 2, "g_8", function="sigmoid")
#        layers.append(g_8)
